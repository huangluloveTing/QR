...
/*
 *    用于临时存储密码值，当第二次输入和第一次相同时，才保存到本机上
 */
@property(nonatomic ,copy) NSString *tempPaswd ;
...
+(instancetype)lockOnView:(UIView*)view delegate:(id<DBGuestureLockDelegate>)delegate {
    ...
    CGRect frame = CGRectMake(0, height - width, width, width);
...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) { // Draw 9 Lock Buttons
        self.tempPaswd = [NSString string];
...
-(void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if ((correctPswd == nil && self.tempPaswd.length < 4) || password.length < 4) {
        self.tempPaswd = password;
        [self.delegate guestureLock:self didSetPassword:password];
        [self setPropertiesByState:DBButtonStateNormal];
    
    } else if ([password isEqualToString:self.tempPaswd]) {
